...
<body class="bg-slate-100 h-screen flex flex-col font-sans" x-data="posSystem()">
    ...
    <header class="bg-slate-900 text-white px-6 py-4 shadow-md flex justify-between items-center">
        <div class="flex items-center space-x-3">
            ...
            <h1 class="text-xl font-bold tracking-wide">Restaurant POS & Inventory</h1>
        </div>
    ...
    <div class="flex-1 flex overflow-hidden">
        ...
        <div class="w-2/3 p-6 overflow-y-auto">
            <h2 class="text-lg font-bold text-slate-700 mb-4">Mga Pagkain at Inumin</h2>

            <div class="grid grid-cols-3 gap-5">
                @forelse($menuItems as $item)
                    @php
                        // Paghahanda ng tiyak na Unsplash fallback image batay sa pangalan ng pagkain
                ...
                @empty
                    <div class="col-span-3 bg-white p-12 rounded-2xl text-center text-slate-400 border border-slate-200 shadow-sm">
                        Walang mahanap na menu items sa database.
                    </div>
                @endforelse
            </div>
        </div>
        ...
        <div class="w-1/3 bg-white border-l border-slate-200 p-6 flex flex-col justify-between shadow-lg">
            ...
            <!-- Total at Checkout Button -->
            <div class="border-t border-slate-200 pt-4">
                <div class="flex justify-between items-baseline mb-4">
                    <span class="text-slate-600 font-medium">Kabuuang Bayarin:</span>
                ...
                <button @click="checkout()" 
                    ...
                    <span x-show="!loading">Bayaran (Checkout)</span>
                    <span x-show="loading" class="animate-pulse">Ipinoproseso...</span>
                </button>
            </div>
        </div>
    </div>
                ...
                async checkout() {
                    ...
                    try {
                        ...
                        if (response.ok && data.success) {
                            alert(data.message || 'Tagumpay ang order!');
                            this.cart = [];
                            location.reload();
                        } else {
                            alert('Error: ' + (data.message || 'May nangyaring mali sa server.'));
                        }
                    } catch (error) {
                        alert('Error: Nagkaroon ng problema sa pag-connect sa server.');
                    } finally {
                        this.loading = false;
                    }
                }
