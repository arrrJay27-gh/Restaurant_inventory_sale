...
<body class="bg-slate-100 h-screen flex flex-col font-sans" x-data="posSystem()">
    ...
    <header class="bg-slate-900 text-white px-6 py-4 shadow-md flex justify-between items-center">
        <div class="flex items-center space-x-3">
            <svg class="w-7 h-7 text-[#70C116]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="flex items-center space-x-3">
            <!-- ADMIN ONLY: Add New Product Button -->
            @if(auth()->check() && strtolower(auth()->user()->role) === 'admin')
                <a href="{{ route('items.create') }}" class="bg-[#70C116] hover:bg-[#5f9e12] text-white text-xs px-3.5 py-2 rounded-lg font-bold transition flex items-center space-x-1.5 shadow-sm">
            ...
            <div class="text-xs bg-slate-800 text-slate-300 px-3 py-2 rounded-lg border border-slate-700 hidden sm:block font-medium">
                System Online
            </div>

            <!-- Home / Landing Page Button -->
            <a href="{{ url('/') }}" class="bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs px-3 py-2 rounded-lg border border-slate-700 font-semibold transition flex items-center space-x-1">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <form method="POST" action="{{ route('logout') }}" class="inline">
                @csrf
                <button type="submit" class="bg-red-600/80 hover:bg-red-600 text-white text-xs px-3 py-2 rounded-lg font-semibold transition flex items-center space-x-1">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="flex-1 flex overflow-hidden">
        ...
        <div class="w-2/3 p-6 overflow-y-auto">
            <div class="flex justify-between items-center mb-4">
                <h2 class="text-lg font-bold text-slate-700">Food & Drinks</h2>
                <span class="text-xs text-slate-500 font-semibold">Select items to add to order</span>
            </div>
                ...
                @forelse($menuItems as $item)
                    ...
                    <div @click="addToCart({{ json_encode($item) }})" 
                        ...
                        <div class="p-4 flex flex-col justify-between flex-1">
                            <div>
                                <h3 class="font-bold text-slate-800 text-base group-hover:text-[#70C116] transition-colors line-clamp-1">
                                    {{ $item->name }}
                                </h3>
                            </div>
                            <div class="mt-3 flex justify-between items-center">
                                <span class="text-[#70C116] font-extrabold text-lg">₱{{ number_format($item->price, 2) }}</span>
                                <span class="text-xs bg-emerald-50 text-[#70C116] font-semibold px-2.5 py-1 rounded-full border border-emerald-200">
                                    + Add
                                </span>
                            </div>
                        </div>
        ...
        <div class="w-1/3 bg-white border-l border-slate-200 p-6 flex flex-col justify-between shadow-lg">
            ...
            <div class="border-t border-slate-200 pt-4">
                ...
                <button @click="checkout()" 
                        ...
                        :class="cart.length === 0 || loading ? 'bg-slate-300 cursor-not-allowed' : 'bg-[#70C116] hover:bg-[#5f9e12] shadow-lg shadow-emerald-600/20'"
